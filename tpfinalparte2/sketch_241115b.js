...
function perdiste(texto, imagen) {
  image(imagen, 0, 0)
    text(texto, 320, 50)
}

function ganaste(texto, imagen) {
  image(imagen, 0, 0)
    text(texto, 320, 50)
}
...
function draw() {
  ...
  if (escenas[0] == 0) {
    ...
    game("derrota a freezer", escenario[1]);
  } else if (escenas[2]==0) {
    ganaste("HAS VENCIDO A FREEZER", escenario[2]);
  } else if (escenas[3]==0) {
    game("FREEZER TE HA MATADO", escenario[3]);
  }
}
